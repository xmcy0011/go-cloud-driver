...
//go:generate mockgen -package mock -source ./dbaccess.go -destination ./mock/dbaccess.go
...
type DBMetadataClosure interface {
	...
	// MoveLargeSubTree：移动超大子树到另外一个节点
	MoveLargeSubTree(ctx context.Context, objectId, parentId string, pageSize int) (deleteCount, insertCount int64, err error)
